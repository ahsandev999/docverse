...
export default function SignUpPage() {
  ...
  const { signUp, isLoading, isClerkEnabled } = useAuth();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 px-4">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-1/4 right-1/4 w-96 h-96 rounded-full bg-indigo-100/40 dark:bg-indigo-900/20 blur-[100px]" />
        <div className="absolute bottom-1/4 left-1/4 w-96 h-96 rounded-full bg-violet-100/40 dark:bg-violet-900/20 blur-[100px]" />
      </div>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2 font-heading">Create your account</h1>
          <p className="text-slate-500 dark:text-slate-400">Start transforming your documents today</p>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-xl shadow-slate-200/50 dark:shadow-none">
          <button
            type="button"
            onClick={() => signUp('Google User', 'user@google.com', 'google-oauth')}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="signup-name" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Full Name</label>
            ...
            <div>
              <label htmlFor="signup-email" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Email</label>
              <div className="relative">
          ...
          <p className="mt-4 text-xs text-slate-400 dark:text-slate-500 text-center">By creating an account, you agree to our Terms of Service and Privacy Policy.</p>
          <div className="mt-6 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400">Already have an account? <Link to="/sign-in" className="text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Sign in</Link></p>
          </div>
        </div>
      </motion.div>
    </div>
